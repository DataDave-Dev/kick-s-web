...
const filtrosContainer =
    document.getElementById("filtrosContainer");
...
function filtrarProductos() {
    ...
    tarjetas.forEach((tarjeta) => {
        ...
        const precioProducto = parseFloat(
            tarjeta.getAttribute("data-precio"),
        );
        ...
        const coincidePrecio =
            precioProducto >= precioMin &&
            precioProducto <= precioMax;
...
function ordenarProductos(productosVisibles) {
    ...
    productosVisibles.sort((a, b) => {
        if (orden === "precioAsc") {
            return (
                parseFloat(a.getAttribute("data-precio")) -
                parseFloat(b.getAttribute("data-precio"))
            );
            ...
            return a
                ...
                .localeCompare(b.getAttribute("data-nombre"));
        } else if (orden === "nombreDesc") {
            return b
                .getAttribute("data-nombre")
                .localeCompare(a.getAttribute("data-nombre"));
        }
        return 0;
    });
...
function toggleFiltrosMovil() {
    ...
    if (isHidden) {
        ...
        toggleFiltros.querySelector("span").textContent =
            "Ocultar filtros";
    } else {
...
window.addEventListener("resize", () => {
    if (window.innerWidth >= 1024) {
        filtrosContainer.classList.remove("hidden");
    } else if (!filtrosContainer.classList.contains("hidden")) {
    }
});